...
class Simultaneous<Event, State> implements StreamTransformer<Event, State> {
  ...
  */
  ...
  const Simultaneous(Stream<State> Function(Event) convert,
  ...
  const Simultaneous.unlimited(Stream<State> Function(Event) convert,
  ...
  const Simultaneous.single(Stream<State> Function(Event) convert,
      {bool cancelOnError = false})
      : assert(convert != null),
        _convert = convert,
        _maxProcesses = 1,
  ...
  const Simultaneous.pair(Stream<State> Function(Event) convert,
      {bool cancelOnError = false})
      : assert(convert != null),
        _convert = convert,
        _maxProcesses = 2,
  ...
  Stream<State> bind(Stream<Event> events) {
    final _SimultaneousController controller =
    ...
    // ITERATOR +
    Future.doWhile(
      () => eventIterator.moveNext().then((bool hasNext) async {
        if (!hasNext) {
