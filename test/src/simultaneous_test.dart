...
Stream<T> _pongDelayed<T>(T value) => Future<T>.delayed(
    const Duration(milliseconds: _delayedOperationInMilliseconds),
    () => value).asStream();

void main() {
  test('shouldRun', () async {
    const StreamTransformer<int, int> simultaneous =
        Simultaneous<int, int>(_pong);
  ...
  test('shouldRunAsDelayed', () async {
    const StreamTransformer<int, int> simultaneous =
        Simultaneous<int, int>(_pongDelayed);
    await _sourceStream.transform<int>(simultaneous).drain<int>();
    expect(true, true);
  }, timeout: _smallTimeout);
  ...
  test('shouldReturnExpectedData', () async {
    const StreamTransformer<int, int> simultaneous =
        Simultaneous<int, int>(_pong);
    List<int> result =
        await _sourceStream.transform<int>(simultaneous).toList();
  ...
  test('shouldBeDelayedAsSingle', () async {
    ...
    expect(
        dt <
            _delayedOperationInMilliseconds * _sourceList.length +
                _toleranceInMilliseconds,
        true);
  }, timeout: _smallTimeout);
  ...
  test('shouldBeDelayedAsPair', () async {
    final int begin = DateTime.now().millisecondsSinceEpoch;
    const StreamTransformer<int, int> simultaneous =
        Simultaneous<int, int>.pair(_pongDelayed);
    await _sourceStream.transform<int>(simultaneous).drain<int>();
    final int end = DateTime.now().millisecondsSinceEpoch;
    final int dt = end - begin;
    expect(
        dt >= _delayedOperationInMilliseconds * (_sourceList.length / 2).ceil(),
        true);
    expect(
        dt <
            _delayedOperationInMilliseconds * (_sourceList.length / 2).ceil() +
                _toleranceInMilliseconds,
        true);
  }, timeout: _smallTimeout);
  ...
  test('shouldBeDelayedAsCustom', () async {
    ...
    const StreamTransformer<int, int> simultaneous = Simultaneous<int, int>(
        _pongDelayed,
    ...
    expect(
        dt >=
            _delayedOperationInMilliseconds *
                (_sourceList.length / maxNumberOfProcesses).ceil(),
        true);
    expect(
        dt <
            _delayedOperationInMilliseconds *
  ...
  test('cancelOnErrorFalse', () async {
    const StreamTransformer<int, int> simultaneous =
        Simultaneous<int, int>(_pong);
    bool hasError = false;
    await Stream<int>.error(Exception('test'))
        .transform<int>(simultaneous)
        .handleError((dynamic _) => hasError = true)
        .drain<int>();
  ...
  test('cancelOnErrorTrue', () async {
    const StreamTransformer<int, int> simultaneous =
        Simultaneous<int, int>(_pong, cancelOnError: true);
    bool hasError = false;
    await Stream<int>.error(Exception('test'))
        .transform<int>(simultaneous)
        .handleError((dynamic _) => hasError = true)
  ...
  test('emptySourceStream', () async {
    const StreamTransformer<void, void> simultaneous =
        Simultaneous<void, void>(_pong);
    await const Stream<void>.empty()
        .transform<void>(simultaneous)
        .drain<void>();
    expect(true, true);
  }, timeout: _smallTimeout);
}
